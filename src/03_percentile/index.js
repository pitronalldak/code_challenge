...
exports.percentile = function(percentile, data) {
  const percentileIndex = Math.ceil((percentile / 100) * data.length) - 1;
  const sortedData = data.sort((x, y) => x - y);

  return sortedData.filter((item, index) => index >= percentileIndex);
};
